...
def _remote_sdk(ctx, urls, strip_prefix, sha256):
    ...
    # If the user provides a local Go SDK, use it.
    local_go_sdk = ctx.os.environ.get("ZOS_GO_SDK")

    if local_go_sdk:
        ctx.report_progress("Using local Go SDK: {}".format(local_go_sdk))
        ...
        if res.return_code:
            fail("error extracting local Go SDK:\n" + res.stdout + res.stderr)

        # IBM Go pax extracts into ./go
        res = ctx.execute([
            "mv",
            "go/*",
            ".",
        ...
        if res.return_code:
            fail("error moving Go SDK:\n" + res.stdout + res.stderr)

        ctx.delete("go")
        return

    # Default behaviour (download)
    ctx.report_progress("Downloading and extracting Go toolchain")
